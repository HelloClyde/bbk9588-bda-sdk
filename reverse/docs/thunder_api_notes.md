...
| SDK API | 参数/行为 |
| --- | --- |
| `bda_sys_package_sound_op40_like(sound_id)` | 把 id 限制到 0..`0x62`，写全局 sound id 并置 pending flag。 |
| `bda_sys_package_sound_op44_like()` | 无参数触发内部 helper；无稳定返回值。 |
...
`SYS+0x050/+0x054` 都是 `return 1` stub，不要作为 load/init/free API 调用。
