...
8013 虽可直接读取 750 kHz TCU1 和 168 MHz CP0 Count，但真机 V2 的 TCU MMIO
全部返回全一值，CP0 Count 恒为零；V3 的旧固件 RAM counter 在 8013 也始终为零。
继续扫描函数表后找到了标称 1 ms API：GUI `+0x714` 启动 TCU0/IRQ `0x17`，
`+0x71c` 返回 32-bit counter，`+0x718` 停止并注销 IRQ。V4 在 8013 和真机均通过
start/read/stop 生命周期；真机 200 ms 窗口实测 `200/194/194/194`，subtick 和 stop
稳定检查通过。该组接口现已进入公开 `bda_time.h`，但不承诺精确墙钟误差。
