...
static const char k_log_path_a[] =
    "A:\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEANIM.TXT";
static const char k_log_path_root[] =
    "\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEANIM.TXT";
static const char k_window_title[] = "IMGV8";
...
static void init_stage_header(void) {
    int index;

    bda_memset(g_stage, 0, sizeof(g_stage));
    g_stage[0] = 'V';
    g_stage[1] = 'X';
    for (index = 2; index < 6; ++index) {
        g_stage[index] = 0xcc;
    }
    write_u32_le(g_stage + 6, STAGE_WIDTH);
    write_u32_le(g_stage + 10, STAGE_HEIGHT);
...
static void build_stage(u32 frame) {
    int x;
    int y;
    int travel = (int)(frame % 352u);
    int sprite_x = travel <= 176 ? travel : 352 - travel;
    u16 background = rgb565(10, 22, 36);
    u16 grid = rgb565(22, 58, 76);
    u16 sprite = ((frame / 60u) & 1u)
        ? rgb565(250, 190, 35)
        : rgb565(25, 210, 200);
    ...
    for (y = 0; y < STAGE_HEIGHT; ++y) {
        for (x = 0; x < STAGE_WIDTH; ++x) {
            ...
            if (x >= sprite_x && x < sprite_x + 32 && y >= 16 && y < 48) {
                int local_x = x - sprite_x;
                int local_y = y - 16;

                if (local_x == local_y || local_x + local_y == 31 ||
                    (local_x >= 12 && local_x < 20) ||
...
static void draw_static_scene(void) {
    ...
    (void)bda_gui_draw_text_like(g_draw, 25, 8, "COMPAT ANIMATION V8", -1);
    (void)bda_gui_draw_text_like(g_draw, 55, 55, "208 x 64 OFFSCREEN", -1);
    ...
    (void)bda_gui_draw_text_like(
        ...
        35,
        205,
        g_animation_complete ? "240 FRAMES COMPLETE" : "ANIMATION RUNNING",
        -1
    );
    (void)bda_gui_set_text_color_like(g_draw, foreground);
...
static int draw_animation_frame(void) {
    ...
    if (!g_draw || !g_compat || !g_draw_object) {
        return 0;
    }

    build_stage(g_animation_frame);
    draw_result = bda_gui_draw_vx_like(g_compat, 0, 0, g_stage);
    guard_begin_result = bda_gui_draw_guard_begin_like();
    old_object = bda_gui_select_draw_object_like(g_draw, g_draw_object);
    copy_result = bda_gui_context_copy_like(
        g_compat,
        0,
        0,
        STAGE_WIDTH,
    ...
    if (g_animation_frame == 0) {
        log_value("FIRST DRAW=", (u32)draw_result);
        log_value("FIRST GUARD BEGIN=", (u32)guard_begin_result);
        log_value("FIRST COPY=", (u32)copy_result);
...
int bda_main(void) {
    ...
    reset_log();
    log_text("START COMPAT ANIMATION V8");
    init_stage_header();
    ...
    log_text("END COMPAT ANIMATION V8");
