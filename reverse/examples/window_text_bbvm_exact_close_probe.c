...
static void draw_exact_text(void) {
    if (!g_draw) {
        return;
    }
    ...
    bda_gui_draw_text_like(g_draw, 18, 24, "BBVM EXACT CLOSE", -1);
    bda_gui_draw_text_like(g_draw, 18, 48, "0x66 cleanup exact", -1);
...
int bda_main(void) {
    ...
    if ((s32)g_frame == -1 || !g_frame) {
        bda_msgbox("Exact", "register failed");
        return 1;
    }
