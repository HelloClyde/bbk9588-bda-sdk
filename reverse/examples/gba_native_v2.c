...
static const char *ROM_CFG = "A:\\gba\\gba.cfg";
static const char *ROM_CFG_ALT = "a:\\gba\\gba.cfg";
...
static int load_selected_path(void) {
    char buf[72];
    int f = bda_fs_fopen_raw(ROM_CFG, "rb");
    int got;
    if (!f) {
        f = bda_fs_fopen_raw(ROM_CFG_ALT, "rb");
    }
    if (!f) {
        copy_text(g_selected_path, ROM_PATH, sizeof(g_selected_path));
        return 0;
    }
    bda_memset(buf, 0, sizeof(buf));
    got = bda_fs_fread_raw(buf, 1, sizeof(buf) - 1, f);
    bda_fs_close_raw(f);
    if (got <= 0) {
        copy_text(g_selected_path, ROM_PATH, sizeof(g_selected_path));
        return 0;
    ...
    if ((buf[1] == ':') || (buf[0] == '\\') || (buf[0] == '/')) {
        copy_text(g_selected_path, buf, sizeof(g_selected_path));
    } else {
        append_path_name(g_selected_path, buf, sizeof(g_selected_path));
    }
    return 1;
}
...
static int load_rom(gba_core_t *g, u8 *hdr, u32 *file_size_out) {
    int f;
    int got;
    u32 size;
    load_selected_path();
...
int bda_main(void) {
    ...
    loaded = load_rom(&g, hdr, &file_size);
    ...
    append_text(&out, end, "GBA native v2\n");
    if (loaded == 0) {
