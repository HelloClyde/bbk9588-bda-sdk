...
#define SPRITE_COLOR_KEY BDA_GUI_COLOR_KEY_MAGENTA_RGB565_LIKE

static const char k_log_path_a[] =
    "A:\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG21.TXT";
static const char k_log_path_root[] =
    "\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG21.TXT";
static const char k_window_title[] = "GFXV21";
...
static void build_sprite(u32 frame) {
    int x;
    int y;
    u16 background = (u16)SPRITE_COLOR_KEY;
...
static void draw_static_scene(void) {
    ...
    (void)bda_gui_draw_text_like(g_draw, 34, 8, "DIRTY RECT SPRITE V21", -1);
    (void)bda_gui_draw_text_like(g_draw, 28, 55, "DIRTY RESTORE + PRESENT", -1);
    ...
    (void)bda_gui_draw_text_like(
        ...
        35,
        205,
        g_animation_complete ? "240 FRAMES COMPLETE" : "PARTIAL PRESENT",
        -1
    );
    (void)bda_gui_set_text_color_like(g_draw, foreground);
...
static int draw_animation_frame(void) {
    ...
    if (!g_draw || !g_clean_context || !g_compat ||
        !g_sprite_context || !g_draw_object) {
        return 0;
    }

    travel = (int)(g_animation_frame % 352u);
    sprite_x = travel <= 176 ? travel : 352 - travel;
    old_sprite_x = g_previous_sprite_x;
    ...
    if (g_animation_frame != 0) {
        restore_result = bda_gui_context_copy_like(
            g_clean_context,
            old_sprite_x,
            16,
            SPRITE_WIDTH,
    ...
    if (g_animation_frame == 0) {
        dirty_left = 0;
        dirty_top = 0;
        dirty_width = STAGE_WIDTH;
        dirty_height = STAGE_HEIGHT;
    } else {
        dirty_left = old_sprite_x < sprite_x ? old_sprite_x : sprite_x;
        dirty_top = 16;
        dirty_right = (old_sprite_x > sprite_x ? old_sprite_x : sprite_x) +
            SPRITE_WIDTH;
        dirty_width = dirty_right - dirty_left;
        dirty_height = SPRITE_HEIGHT;
    }
    ...
    if (g_animation_frame == 0) {
        ...
        log_value("COLOR KEY=", SPRITE_COLOR_KEY);
        log_value("FIRST SPRITE COPY=", (u32)sprite_copy_result);
        log_value("FIRST GUARD BEGIN=", (u32)guard_begin_result);
        log_value("FIRST FULL PRESENT=", (u32)present_copy_result);
        log_value("FIRST GUARD END=", (u32)guard_end_result);
    } else if (g_animation_frame == 1) {
        log_value("FIRST RESTORE=", (u32)restore_result);
        log_value("FIRST DIRTY COMPOSE=", (u32)sprite_copy_result);
        log_value("DIRTY LEFT=", (u32)dirty_left);
        log_value("DIRTY WIDTH=", (u32)dirty_width);
        log_value("FIRST DIRTY GUARD BEGIN=", (u32)guard_begin_result);
        log_value("FIRST DIRTY PRESENT=", (u32)present_copy_result);
        log_value("FIRST DIRTY GUARD END=", (u32)guard_end_result);
    }
    g_previous_sprite_x = sprite_x;
    ++g_animation_frame;
...
int bda_main(void) {
    ...
    int clean_draw_result;
    int back_draw_result;
    int close_requested = 0;
    u32 close_wait = 0;

    reset_log();
    log_text("START DIRTY RECT SPRITE PROBE V21");
    ...
    log_text("BEFORE CLEAN CREATE");
    g_clean_context = bda_gui_compat_context_create_like(g_draw);
    log_value("CLEAN=", (u32)g_clean_context);
    if (!g_clean_context || (s32)g_clean_context == -1) {
        log_text("RESULT=CLEAN FAIL");
        (void)bda_gui_close_frame_like(g_frame);
        g_frame = 0;
        return 3;
    }
    ...
    if (!g_compat || (s32)g_compat == -1) {
        ...
        bda_gui_surface_flush_like(g_clean_context);
        g_clean_context = 0;
        (void)bda_gui_close_frame_like(g_frame);
        g_frame = 0;
        return 4;
    ...
    build_stage();
    clean_draw_result = bda_gui_draw_vx_like(
        g_clean_context, 0, 0, g_stage
    );
    back_draw_result = bda_gui_draw_vx_like(g_compat, 0, 0, g_stage);
    log_value("CLEAN DRAW=", (u32)clean_draw_result);
    log_value("BACK DRAW=", (u32)back_draw_result);
    if (clean_draw_result || back_draw_result) {
        ++g_failures;
    }
    draw_static_scene();
    g_need_draw = 0;
    ...
    if (g_clean_context && (s32)g_clean_context != -1) {
        log_text("BEFORE CLEAN FREE");
        bda_gui_surface_flush_like(g_clean_context);
        g_clean_context = 0;
        log_text("CLEAN FREED");
    }
    if (g_compat && (s32)g_compat != -1) {
    ...
    log_text("END DIRTY RECT SPRITE PROBE V21");
    return g_failures ? 6 : 0;
}
