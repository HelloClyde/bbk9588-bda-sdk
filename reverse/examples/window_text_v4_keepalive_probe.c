...
static void draw_keepalive(bda_handle_t window) {
    ...
    bda_gui_draw_text_like(draw, 18, 24, "V4 KEEPALIVE", -1);
    bda_gui_draw_text_like(draw, 18, 48, "redraw after loop", -1);
...
int bda_main(void) {
    ...
    if ((s32)g_window == -1 || !g_window) {
        bda_msgbox("Keep", "register failed");
        return 1;
    }
