...
static const char k_log_path_a[] =
    "A:\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG10.TXT";
static const char k_log_path_root[] =
    "\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG10.TXT";
static const char k_window_title[] = "GFXV10";

static const bda_point_like_t k_zigzag[] = {
    {16, 78}, {50, 118}, {84, 78}, {118, 118}, {152, 78}, {186, 118}, {224, 78}
};
static const bda_point_like_t k_diamond[] = {
    {120, 154}, {202, 216}, {120, 278}, {38, 216}, {120, 154},
    {120, 278}, {202, 216}, {38, 216}, {120, 154}
};
static const bda_point_like_t k_inside_point = {120, 160};
static const bda_point_like_t k_outside_point = {-16, -16};
static const bda_rect_like_t k_inside_rect = {24, 72, 216, 280};
static const bda_rect_like_t k_outside_rect = {280, 340, 320, 380};
...
static void record_clip_results(bda_handle_t context) {
    bda_rect_like_t clip;
    int point_in;
    int point_out;
    int rect_in;
    int rect_out;

    bda_memset(&clip, 0, sizeof(clip));
    bda_gui_clip_bounds_like(context, &clip);
    point_in = bda_gui_clip_contains_point_like(context, &k_inside_point);
    point_out = bda_gui_clip_contains_point_like(context, &k_outside_point);
    rect_in = bda_gui_clip_intersects_rect_like(context, &k_inside_rect);
    rect_out = bda_gui_clip_intersects_rect_like(context, &k_outside_rect);

    log_rect("CLIP=", &clip);
    log_value("POINT IN=", (u32)point_in);
    log_value("POINT OUT=", (u32)point_out);
    log_value("RECT IN=", (u32)rect_in);
    log_value("RECT OUT=", (u32)rect_out);

    if (clip.x1 <= clip.x0 || clip.y1 <= clip.y0 ||
        !point_in || point_out || !rect_in || rect_out) {
        ++g_failures;
        log_text("CLIP QUERY=FAIL");
    } else {
        log_text("CLIP QUERY=PASS");
    }
}
...
static void draw_scene(void) {
    ...
    (void)bda_gui_draw_text_like(g_draw, 27, 8, "POLYLINE + CLIP V10", -1);
    (void)bda_gui_set_text_color_like(g_draw, accent);
    (void)bda_gui_draw_text_like(g_draw, 64, 42, "CONNECTED POINTS", -1);
    bda_gui_polyline_like(
        g_draw, k_zigzag, (u32)(sizeof(k_zigzag) / sizeof(k_zigzag[0]))
    );
    bda_gui_polyline_like(
        g_draw, k_diamond, (u32)(sizeof(k_diamond) / sizeof(k_diamond[0]))
    );
    (void)bda_gui_set_text_color_like(g_draw, foreground);
    (void)bda_gui_draw_text_like(g_draw, 72, 296, "ESC EXIT", -1);
    (void)bda_gui_select_draw_object_like(g_draw, old_object);

    if (!g_scene_logged) {
        g_scene_logged = 1;
        record_clip_results(g_draw);
        log_value("POLYLINE CALLS=", 2);
...
int bda_main(void) {
    ...
    reset_log();
    log_text("START GAME POLYLINE CLIP PROBE V10");
    ...
    log_text("END GAME POLYLINE CLIP PROBE V10");
