...
static const char k_log_path_a[] =
    "A:\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG17.TXT";
static const char k_log_path_root[] =
    "\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG17.TXT";
static const char k_window_title[] = "GFXV17";

static const bda_rect_like_t k_left_clip = {30, 80, 95, 225};
static const bda_rect_like_t k_right_clip = {145, 80, 210, 225};
static const bda_rect_like_t k_reported_bounds = {30, 80, 95, 225};
static const bda_rect_like_t k_empty_bounds = {0, 0, 0, 0};
static const bda_point_like_t k_left_point = {60, 150};
static const bda_point_like_t k_right_point = {180, 150};
...
static void probe_and_draw_union(bda_handle_t context) {
    bda_rect_like_t union_bounds;
    ...
    bda_gui_clip_bounds_like(context, &union_bounds);
    ...
    if (!rect_equals(&union_bounds, &k_reported_bounds)) {
        ++g_failures;
    }
    if (!left_hit || !right_hit || gap_hit || outside_hit || gap_rect_hit ||
        !crossing_rect_hit) {
        ++g_failures;
    }
    ...
    bda_gui_move_to_like(context, 5, 60);
    bda_gui_line_to_like(context, 235, 245);
    bda_gui_move_to_like(context, 235, 60);
    ...
    if (!g_scene_logged) {
        log_rect("REPORTED BOUNDS=", &union_bounds);
...
static void draw_scene(void) {
    ...
    (void)bda_gui_draw_text_like(g_draw, 45, 7, "CLIP UNION V17", -1);
    (void)bda_gui_set_text_color_like(g_draw, accent);
    (void)bda_gui_draw_text_like(g_draw, 39, 35, "TWO ISLANDS + GAP", -1);

    probe_and_draw_union(g_draw);
    ...
    if (!g_scene_logged) {
        ...
        log_text(g_failures ? "CLIP UNION=FAIL" : "CLIP UNION=PASS");
    }
...
int bda_main(void) {
    ...
    reset_log();
    log_text("START GAME CLIP UNION PROBE V17");
    ...
    log_text("END GAME CLIP UNION PROBE V17");
