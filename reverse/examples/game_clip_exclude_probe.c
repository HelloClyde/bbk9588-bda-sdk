...
static const char k_log_path_a[] =
    "A:\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG16.TXT";
static const char k_log_path_root[] =
    "\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG16.TXT";
static const char k_window_title[] = "GFXV16";

static const bda_rect_like_t k_base_clip = {30, 70, 210, 230};
static const bda_rect_like_t k_hole = {85, 110, 155, 190};
static const bda_rect_like_t k_empty_bounds = {0, 0, 0, 0};
static const bda_point_like_t k_ring_point = {50, 150};
static const bda_point_like_t k_hole_point = {120, 150};
static const bda_point_like_t k_outside_point = {15, 150};
static const bda_rect_like_t k_hole_rect = {100, 130, 140, 170};
static const bda_rect_like_t k_crossing_rect = {70, 130, 100, 170};
...
static void probe_and_draw_exclude(bda_handle_t context) {
    bda_rect_like_t excluded_bounds;
    ...
    int restored_hole_hit;
    s32 y;

    bda_gui_clip_select_rect_like(context, &k_base_clip);
    bda_gui_clip_exclude_rect_like(
        context, k_hole.x0, k_hole.y0, k_hole.x1, k_hole.y1
    );
    bda_gui_clip_bounds_like(context, &excluded_bounds);

    ring_hit = bda_gui_clip_contains_point_like(context, &k_ring_point);
    hole_hit = bda_gui_clip_contains_point_like(context, &k_hole_point);
    outside_hit = bda_gui_clip_contains_point_like(context, &k_outside_point);
    hole_rect_hit = bda_gui_clip_intersects_rect_like(context, &k_hole_rect);
    crossing_rect_hit = bda_gui_clip_intersects_rect_like(
        context, &k_crossing_rect
    );

    if (!rect_equals(&excluded_bounds, &k_base_clip)) {
        ++g_failures;
    }
    if (!ring_hit || hole_hit || outside_hit || hole_rect_hit ||
        !crossing_rect_hit) {
        ++g_failures;
    }
    ...
    bda_gui_move_to_like(context, 5, 55);
    bda_gui_line_to_like(context, 235, 245);
    bda_gui_move_to_like(context, 235, 55);
    bda_gui_line_to_like(context, 5, 245);

    bda_gui_clip_select_rect_like(context, 0);
    bda_gui_clip_bounds_like(context, &restored_bounds);
    restored_hole_hit = bda_gui_clip_contains_point_like(
        context, &k_hole_point
    );
    ...
    draw_cross(context, k_hole_point.x, k_hole_point.y, 9);

    if (!g_scene_logged) {
        log_rect("EXCLUDED BOUNDS=", &excluded_bounds);
        log_value("RING POINT=", (u32)ring_hit);
        log_value("HOLE POINT=", (u32)hole_hit);
        log_value("OUTSIDE POINT=", (u32)outside_hit);
        log_value("HOLE RECT=", (u32)hole_rect_hit);
        log_value("CROSSING RECT=", (u32)crossing_rect_hit);
        log_rect("RESTORED=", &restored_bounds);
        log_value("RESTORED HOLE POINT=", (u32)restored_hole_hit);
    }
}
...
static void draw_scene(void) {
    ...
    (void)bda_gui_draw_text_like(g_draw, 37, 7, "CLIP EXCLUDE V16", -1);
    (void)bda_gui_set_text_color_like(g_draw, accent);
    (void)bda_gui_draw_text_like(g_draw, 44, 35, "CENTER HOLE 70 X 80", -1);

    probe_and_draw_exclude(g_draw);

    (void)bda_gui_set_text_color_like(g_draw, foreground);
    (void)bda_gui_draw_text_like(g_draw, 28, 252, "CENTER CROSS = RESET OK", -1);
    ...
    if (!g_scene_logged) {
        ...
        log_text(g_failures ? "CLIP EXCLUDE=FAIL" : "CLIP EXCLUDE=PASS");
    }
...
int bda_main(void) {
    ...
    reset_log();
    log_text("START GAME CLIP EXCLUDE PROBE V16");
    ...
    log_text("END GAME CLIP EXCLUDE PROBE V16");
