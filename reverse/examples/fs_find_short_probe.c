...
int bda_main(void) {
    ...
    for (unsigned int i = 0; i < sizeof(g_cases) / sizeof(g_cases[0]); ++i) {
        ...
        append_line(&out, g_cases[i].name, bda_fs_findfirst_like(g_cases[i].pattern, g_cases[i].attr, g_find_data));
    }

    append_char(&out, 0);
    bda_msgbox("FSFindShort", g_message);
    return 0;
}
