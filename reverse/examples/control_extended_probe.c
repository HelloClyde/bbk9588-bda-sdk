...
static const char k_log_path_a[] = "A:\\CONTROL2.LOG";
static const char k_log_path_root[] = "\\CONTROL2.LOG";
static const char k_window_title[] = "EXTENDED CONTROLS";
static const control_spec_t k_specs[CONTROL_COUNT] = {
    {"sledit", "SLEDIT", 0x08000001u, 0, 0x221u, 12, 18, 216, 26},
    {"mledit", "MLEDIT", 0x08083001u, 0, 0x222u, 12, 52, 216, 48},
};
...
static char g_sledit_text[32];
static char g_mledit_text[32];
...
static bda_handle_t control_create(
    ...
    return fn(
        spec->class_name, spec->caption, spec->style, spec->flags, spec->id,
        spec->x, spec->y, spec->width, spec->height, parent, 0
    );
}
...
static void create_controls(void) {
    ...
    if (handle_is_valid(g_controls[0])) {
        (void)control_send(g_controls[0], CONTROL_MSG_SET_MAX_LENGTH, 31, 0);
        log_value(
            "SET SLEDIT=",
            (u32)control_send(
                g_controls[0], CONTROL_MSG_SET_TEXT, 0, (u32)"SLEDIT READY"
            )
        );
    }
    if (handle_is_valid(g_controls[1])) {
        (void)control_send(g_controls[1], CONTROL_MSG_SET_MAX_LENGTH, 31, 0);
        log_value(
            "SET MLEDIT=",
            (u32)control_send(
                g_controls[1], CONTROL_MSG_SET_TEXT, 0, (u32)"MLEDIT READY"
            )
        );
    }
}
...
static void read_back_text(void) {
    if (handle_is_valid(g_controls[0])) {
        bda_memset(g_sledit_text, 0, sizeof(g_sledit_text));
        log_value(
            "GET SLEDIT=",
            (u32)control_send(
                g_controls[0], CONTROL_MSG_GET_TEXT,
                sizeof(g_sledit_text), (u32)g_sledit_text
            )
        );
        log_text(g_sledit_text);
    }
    if (handle_is_valid(g_controls[1])) {
        bda_memset(g_mledit_text, 0, sizeof(g_mledit_text));
        log_value(
            "GET MLEDIT=",
            (u32)control_send(
                g_controls[1], CONTROL_MSG_GET_TEXT,
                sizeof(g_mledit_text), (u32)g_mledit_text
            )
        );
        log_text(g_mledit_text);
    }
}
...
static int extended_window_proc(
    bda_handle_t handle,
    u32 message,
    u32 wparam,
    u32 lparam
...
int bda_main(void) {
    ...
    g_event_count = 0;
    reset_log();
    log_text("START EXTENDED CONTROLS V2");

    descriptor.style = 0;
    descriptor.title = k_window_title;
    descriptor.wndproc = extended_window_proc;
