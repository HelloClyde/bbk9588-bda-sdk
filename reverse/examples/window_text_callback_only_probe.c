...
static void draw_callback_text(bda_handle_t window) {
    ...
    bda_gui_draw_text_like(draw, 20, 30, "CALLBACK TEXT OK", -1);
...
static int probe_window_proc(bda_handle_t handle, u32 message, u32 wparam, u32 lparam) {
    if (message == 1 || message == 2 || message == 0x66) {
        draw_callback_text(handle);
    }
...
int bda_main(void) {
    ...
    if ((s32)g_window == -1 || !g_window) {
        bda_msgbox("CbText", "register failed");
        return 1;
    }
