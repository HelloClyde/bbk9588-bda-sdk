...
static const char k_log_path_a[] =
    "A:\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG11.TXT";
static const char k_log_path_root[] =
    "\\\xd3\xa6\xd3\xc3\\\xca\xfd\xbe\xdd\\\xd3\xce\xcf\xb7\\GAMEG11.TXT";
static const char k_window_title[] = "GFXV11";
...
static void draw_ellipse_cell(
    ...
    void *old_object = bda_gui_select_draw_object_like(context, object);

    bda_gui_ellipse_like(context, center_x, center_y, 46, 31, filled);
    (void)bda_gui_select_draw_object_like(context, old_object);
}
...
static void draw_scene(void) {
    ...
    old_fill = bda_gui_set_fill_color_like(g_draw, accent);
    (void)bda_gui_set_text_mode_like(g_draw, 1);
    (void)bda_gui_set_text_color_like(g_draw, foreground);
    (void)bda_gui_draw_text_like(g_draw, 40, 7, "ELLIPSE MODES V11", -1);
    (void)bda_gui_set_text_color_like(g_draw, accent);
    (void)bda_gui_draw_text_like(g_draw, 34, 39, "OBJ7 O", -1);
    (void)bda_gui_draw_text_like(g_draw, 150, 39, "OBJ7 F", -1);
    draw_ellipse_cell(g_draw, g_object7, 60, 101, 0);
    draw_ellipse_cell(g_draw, g_object7, 180, 101, 1);

    (void)bda_gui_set_text_color_like(g_draw, accent);
    (void)bda_gui_draw_text_like(g_draw, 34, 153, "OBJ8 O", -1);
    (void)bda_gui_draw_text_like(g_draw, 150, 153, "OBJ8 F", -1);
    draw_ellipse_cell(g_draw, g_object8, 60, 215, 0);
    draw_ellipse_cell(g_draw, g_object8, 180, 215, 1);
    ...
    if (!g_scene_logged) {
        ...
        log_value("FILL OLD=", (u32)old_fill);
        log_value("ELLIPSE CALLS=", 4);
...
int bda_main(void) {
    ...
    reset_log();
    log_text("START GAME ELLIPSE PROBE V11");
    ...
    if (!g_draw || !g_object7 || !g_object8 ||
        (s32)(u32)g_object7 == -1 || (s32)(u32)g_object8 == -1) {
    ...
    log_text("END GAME ELLIPSE PROBE V11");
