...
static void draw_text_v2(bda_handle_t window) {
    ...
    bda_gui_draw_text_like(draw, 18, 24, "V2 AFTER DEFAULT", -1);
    bda_gui_draw_text_like(draw, 18, 48, "activate 0x100", -1);
...
int bda_main(void) {
    ...
    if ((s32)g_window == -1 || !g_window) {
        bda_msgbox("WinV2", "register failed");
        return 1;
    }
