...
static void draw_v4(bda_handle_t window) {
    ...
    bda_gui_draw_text_like(draw, 18, 24, "V4 66 INTERCEPT", -1);
    bda_gui_draw_text_like(draw, 18, 48, "no default no close", -1);
...
int bda_main(void) {
    ...
    if ((s32)g_window == -1 || !g_window) {
        bda_msgbox("WinV4", "register failed");
        return 1;
    }
