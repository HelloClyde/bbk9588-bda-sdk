...
static u32 gba_step_arm(gba_core_t *g) {
    ...
    if ((op & 0x0ffffff0u) == 0x012fff10u) {
        c->r[15] = c->r[op & 0xfu] & ~1u;
        c->steps++;
        return 1;
    }

    if ((op & 0x0c000000u) == 0x00000000u && (op & 0x02000000u)) {
        u32 opcode = (op >> 21) & 0xfu;
        u32 set_flags = (op >> 20) & 1u;
        u32 rn = (op >> 16) & 0xfu;
        u32 rd = (op >> 12) & 0xfu;
        u32 imm = ror32(op & 0xffu, ((op >> 8) & 0xfu) * 2u);
        u32 a = c->r[rn];
        u32 result = 0;
...
static int load_rom(gba_core_t *g, u8 *hdr, u32 *file_size_out) {
    int f = bda_fs_fopen_raw(ROM_PATH, "rb");
    int got;
    u32 size;
    if (!f) {
...
int bda_main(void) {
    ...
    append_text(&out, end, "GBA native v1\n");
    if (loaded == 0) {
