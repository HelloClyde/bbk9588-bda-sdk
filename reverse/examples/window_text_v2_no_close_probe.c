...
static void draw_text_nc(bda_handle_t window) {
    ...
    bda_gui_draw_text_like(draw, 18, 24, "NO CLOSE FRAME", -1);
    bda_gui_draw_text_like(draw, 18, 48, "return after loop", -1);
...
int bda_main(void) {
    ...
    if ((s32)g_window == -1 || !g_window) {
        bda_msgbox("NoClose", "register failed");
        return 1;
    }
