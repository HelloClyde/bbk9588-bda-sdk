...
`SYS+0x044` 返回 effective PCM attenuation 并保存到 `0x81c1288c`。后续
`SYS+0x040` 接收该值或下列形式计算新的 attenuation：

```text
attenuation = 0x75 - (index * 13)
```

该值会被 firmware clamp 到 `0..98`，并在下一次 PCM write 时量化到 `0..96`。
数值越大声音越小，这与 `GameVolV1` 的 PCM 峰值测试一致。
...
`决战坦克.bda` 证明 `SYS+0x058..0x068` 不是 `雷霆战机` 的一次性 helper，而是
内置原生游戏可复用的系统级打包音效接口；`+0x040/+0x044` 则是共享 PCM 衰减控制。
