...
GUI +0x1a4/+0x1a8  control/object 创建和销毁
GUI +0x1ac/+0x1b0  window timer 启动和停止
...
这组参数的真实语义是：在 handle 上启动 timer id 100、周期 400 ms，再停止同一
timer。C200 已确认 `GUI+0x1ac` 经内部 `0x162` 注册，`GUI+0x1b0` 经内部 `0x163`
注销；到期后 Frame callback 收到 message `0x144`、wparam 100。它们不是
lock/unlock、begin/end frame 或对象 update notification。
...
2. `GUI+0x1ac/+0x1b0` 是 window timer start/stop；九门课程使用 timer id 100、
   400 ms 周期，公开 wrapper 已由独立 V4 probe 验证。
3. `GUI+0x3f8/+0x400` 不应标成游戏专用。
4. `FS+0x064` 依赖 firmware 内部 volume/index 和 block 参数，不能暴露为公共 SDK helper。

## 未确认点

1. timer id 100 在九门课程滚动/翻页路径中驱动的具体业务状态。
2. 课程数据文件的结构/记录格式。
