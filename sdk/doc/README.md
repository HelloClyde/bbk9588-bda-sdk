...
搜索路径。只有受控逆向 probe 才显式传入 `-I sdk\api` 覆盖公开 header。
...
下面的综合清单还包含 `sdk/api/bda_sdk.h` 中的逆向候选；带 `_like` 的名称不属于
公开 SDK，普通应用不要依赖：
...
sdk/api/examples/minesweeper_bda.c           娱乐天地分类的 9x9 可玩扫雷；8013 动态通过，真机待复测
