...
仓库自带的 `python -m bda_packer` 会自动把 `sdk\api` 加入 C include
搜索路径；外部构建脚本需要等价传入 `-I sdk\api`。
...
交给 C200，它不是“一次调用显示 UI”的 high-level API；`frame_surface_like(15)` 也只是
取 low-level object/surface，不能单独证明绘图上下文已经准备好。硬编码替换
...
`0x130`，示例 `sdk\api\examples\gui_screen_width_demo.c` 会用 message box 显示结果。

```powershell
python -m bda_packer sdk\api\examples\gui_screen_width_demo.c `
  --title WidthDemo `
  --category 9 `
...
```powershell
python -m bda_packer sdk\api\examples\fs_read_demo.c `
  --title FsRead `
  --category 9 `
  -o build\FsRead.bda
...
`sdk/api/examples/fs_read_raw_demo.c` 则只演示便捷 wrapper
...
`sdk/api/examples/fs_find_demo.c` 是最小 findfirst/findclose 示例，只显示 return value
和结构开头字节，不把某个 pattern 的成功视为跨固件保证。

## Verify 覆盖的 SDK 示例

`reverse.test_sdk_examples` 会编译并 validate 这些 standalone 示例；它们是当前
SDK/toolchain smoke 的推荐起点。开发者示例放在 `sdk/api/examples`，逆向 probe 仍放在
`reverse/examples`：

```text
sdk/api/examples/hello_world_msgbox.c        最小 verified message box
sdk/api/examples/hello_msgbox.c              兼容的 message box build smoke
sdk/api/examples/gui_rect_contains_demo.c    rect helper / RectDemo
sdk/api/examples/gui_screen_width_demo.c     standalone 常量查询 smoke
sdk/api/examples/input_state_demo.c          input packet/event/state 查询
sdk/api/examples/mem_alloc_demo.c            firmware heap alloc/free
sdk/api/examples/fs_read_demo.c              file open/read/seek/tell/close
sdk/api/examples/fs_read_raw_demo.c          read_raw(file, buffer, size) 参数顺序
sdk/api/examples/fs_write_demo.c             已验证的写入、关闭、重开和读回闭环
sdk/api/examples/fs_find_demo.c              directory enumeration struct
sdk/api/examples/fs_diskinfo_demo.c          disk/storage 容量查询
sdk/api/examples/fs_status_demo.c            storage ready + path stat/access
...
`minesweeper_bda.c` 仍含未闭环的窗口和输入实验。它只能由 standalone 打包器做
静态编译检查，不作为可运行游戏；不能借用或 patch 原机游戏，也不能进入
`sdk/doc/verified/` 作为已验证 SDK 用法。

首个在 BBK 9588 真机上完成“创建窗口、触摸绘图、实体键退出、销毁 frame、返回主菜单”
闭环的 standalone BDA 是 `TouchStageV11.bda`。其 API 清单和强制生命周期顺序见
`sdk/doc/verified/touch_window_lifecycle_api.md`。顶层窗口退出必须按
`stop -> release -> event poll 结束 -> close -> bda_main return` 执行。

`reverse/examples` 里其他文件多数是专题 probe 或真机回归实验，使用前先看对应
`sdk/doc/*_notes.md` 和 `reverse/reports/*.md` 的风险说明。
`window_text_bbvm_black_probe.c` 和相邻 BBVM text probe 证明
`BDA_MSG_DRAW_CONTEXT_ATTACH_LIKE` (`0x60`) 中取得 draw context、`0x66` 中清理的
路径可以显示文本；它们仍依赖 `style=0x08000000`、`frame_surface_like(15)` 和完整
event loop，属于 frame/control lifecycle 回归 probe，不是 standalone SDK starter。
