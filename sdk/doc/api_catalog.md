...
本表由 `reverse/bda_api_catalog.py` 生成，合并了 `reverse/reports/bda_inventory.json` 的原机 BDA call inventory 和 `sdk/api/bda_sdk.h` 里的 SDK 命名。
...
| FS | +0x004 | `BDA_FS_CLOSE` | 1171 | 54 | 较高 | fclose-style；C200 单参数 file，return value 来自内部 close helper。 |
| FS | +0x008 | `BDA_FS_READ` | 2658 | 54 | 较高 | fread-style；C200 参数顺序为 buffer,size,count,file，失败返回 0。 |
| FS | +0x00c | `BDA_FS_WRITE` | 2410 | 54 | 较高 | fwrite-style；C200 参数顺序为 buffer,size,count,file，失败返回 0。 |
...
| GUI | +0x088 | `BDA_GUI_FRAME_STOP_LIKE` | 342 | 53 | 中 | 停止 frame/window；C200 只读取 handle，解析 frame 后发送内部 0x66/0xf1 message。 |
| GUI | +0x08c | `BDA_GUI_DEFAULT_PROC_LIKE` | 398 | 54 | 中 | default window procedure fallback；C200 参数为 handle,message,wparam,lparam。 |
| GUI | +0x098 | `BDA_GUI_FRAME_ACTIVATE_LIKE` | 92 | 21 | 中 | 激活/状态切换 frame；C200 参数为 handle,mode，mode 0/0x10/0x100 有特殊路径。 |
...
| GUI | +0x2b8 | `BDA_GUI_MSGBOX` | 381 | 44 | 较高 | message box，hardware probe 已确认可用于简单 BDA demo。 |
...
| GUI | +0x310 | `BDA_GUI_COMPAT_CONTEXT_CREATE_LIKE` | 37 | 13 | 中 | compatible draw context create；C200 分配 0xd4 byte context 并复制 source context 的 drawable bounds/backend。 |
| GUI | +0x314 | `BDA_GUI_SURFACE_FLUSH_LIKE` | 57 | 13 | 中 | surface/canvas flush 并释放 context；C200 调 backend +0x34 后释放 object。 |
| GUI | +0x334 | `BDA_GUI_SET_FILL_COLOR_LIKE` | 56 | 14 | 中 | 设置 fill color；C200 写 context+0x14 并返回旧值。 |
| GUI | +0x338 | `BDA_GUI_SET_TEXT_MODE_LIKE` | 485 | 45 | 中 | 设置文本模式/背景模式；C200 写 context+0x18 并返回旧值。 |
| GUI | +0x33c | `BDA_GUI_SET_TEXT_COLOR_LIKE` | 579 | 44 | 中 | 设置文本颜色；C200 写 context+0x50 并返回旧值。 |
| GUI | +0x358 | `BDA_GUI_SELECT_DRAW_OBJECT_LIKE` | 75 | 22 | 中 | select draw object；C200 把 object 写入 context+0x30 并返回旧值，Thunder 会在绘制后恢复旧 object。 |
...
| GUI | +0x38c | `BDA_GUI_RECTANGLE_LIKE` | 21 | 12 | 中 | rectangle primitive；C200 使用 context,left,top,right,bottom，第五参数从 stack+0x10 读取。 |
...
| GUI | +0x418 | `BDA_GUI_RENDER_FINISH_LIKE` | 117 | 13 | 中 | 双 context/双矩形 render helper；C200 使用 stack+0x14 的第二 context 并调用 backend +0x94。 |
...
| GUI | +0x4f0 | `BDA_GUI_DRAW_TEXT_LIKE` | 1086 | 46 | 中 | draw GBK/ASCII text；C200 使用 handle,x,y,text,extra，extra<0 时按 strlen。 |
| GUI | +0x540 | `BDA_GUI_DRAW_VX_LIKE` | 0 | 0 | 中 | 绘制完整 VX resource block；C200 从第 6 参数读取 resource，尺寸来自 VX header +0x06/+0x0a。 |
| GUI | +0x5d4 | `BDA_GUI_INPUT_PACKET_LIKE` | 0 | 0 | 中 | GAMEBOY/input 按键包 helper；C200 清 6 byte packet 后写入按键状态。 |
| GUI | +0x670 | `BDA_GUI_DECODE_BMP_LIKE` | 0 | 0 | 中 | BMP/VX decode；C200 使用 owner,out,path,out_source_buffer，VX 快路径会写回 file buffer pointer。 |
| GUI | +0x6a8 | `BDA_GUI_FILE_SELECTOR_OPEN_LIKE` | 0 | 0 | 中 | file selector open/session；C200 只读取 a0=mode，内部构造 modal frame。 |
| GUI | +0x6b0 | `BDA_GUI_SCREEN_BUFFER_LIKE` | 0 | 0 | 中 | 内部 screen/framebuffer pointer getter；无参数，不是 allocator；不要直接写或自定义 present。 |
| GUI | +0x6b8 | `BDA_GUI_LIST_NTH_LIKE` | 0 | 0 | 中 | 链表第 N 项 helper；C200 使用 a0=head、a1=index，不是无参数 selector get。 |
| GUI | +0x6bc | `BDA_GUI_LIST_FREE_LIKE` | 0 | 0 | 中 | linked list free helper；C200 将 a0=head 传给 0x8003e868，释放节点和节点 data，不是无参数 selector close。 |
| GUI | +0x6c0 | `BDA_GUI_TOUCH_POSITION_LIKE` | 0 | 0 | 中 | raw-to-logical 触摸坐标转换器；a0/a1 为 u16 output pointer，结果裁剪到 240x320；静态 ABI 已定位，直接 polling 的动态验证无结论，不列入 verified。 |
| GUI | +0x6c8 | `BDA_GUI_FILE_SELECTOR_UPDATE_LIKE` | 0 | 0 | 中 | file selector 更新/pump；C200 table entry 无参数。 |
| GUI | +0x6e0 | `BDA_GUI_GAME_DISPLAY_PUMP_LIKE` | 0 | 0 | 中 | 触摸长按驱动的 game state pump；C200 无参数，先查 pen GPIO，阈值 0x1068 后写全局状态；有副作用。 |
| GUI | +0x72c | `BDA_GUI_STATE_QUERY_LIKE` | 0 | 0 | 中 | GAMEBOY 状态查询；C200 table entry 无参数并更新内部状态 word。 |
| GUI | +0x738 | `BDA_GUI_SCREEN_WIDTH_LIKE` | 0 | 0 | 中 | 返回屏幕宽度常量；C200 当前返回 0x130。 |
| GUI | +0x750 | `BDA_GUI_EVENT_FETCH_LIKE` | 0 | 0 | 中 | event/key 获取；C200 使用 a0/a1 两个输出 pointer，无事件时写 -1。 |
...
| MEM | +0x004 | `BDA_MEM_TRACK_FREE_LIKE` | 1171 | 54 | 中 | tracked heap free；C200 单参数 ptr，debug tracking 开启时清记录后释放。 |
| MEM | +0x008 | `BDA_MEM_ALLOC` | 2658 | 54 | 较高 | 固件堆内存分配/heap alloc；C200 单参数 size，锁保护后返回 pointer。 |
| MEM | +0x00c | `BDA_MEM_FREE` | 2410 | 54 | 较高 | 固件堆内存释放；C200 单参数 ptr，锁保护后调用内部 free。 |
...
| SYS | +0x004 | `BDA_SYS_CLOSE_LIKE` | 1171 | 54 | 中 | 内部 system resource close；C200 使用 resource_id 1..10 查资源表并调用 close callback，不是 app exit。 |
...
| SYS | +0x088 | `BDA_SYS_KEYCODE_RAW_LIKE` | 342 | 53 | 中 | raw keycode query；C200 无参数，读取硬件输入寄存器并返回 raw code。 |
| SYS | +0x08c | `BDA_SYS_AUDIO_RESET_LIKE` | 398 | 54 | 中 | raw audio reset/init；C200 无参数，关闭全局 audio object 后进入初始化路径，无稳定 return value。 |
