...
`元素周期表.bda` 是当前样本中最小的原厂 native BDA，大小 55,900 byte。它很适合作为
...
0x81c004bc  从 app-local resource table 按 index 取 resource pointer
0x81c00654  释放资源表中的附属分配
0x81c0069c  释放 app-local resource table
0x81c006d0  主周期表 window proc
0x81c00f7c  绘制当前元素的小图/局部图
0x81c01204  绘制当前元素在周期表中的高亮/定位图
0x81c014dc  元素详情/浏览 window proc
0x81c0218c  主程序
0x81c0240c  加载 .hx 详情文本 offset table
0x81c02598  按元素 index 从 .hx 读取一段文本到 buffer
0x81c026c0  创建一个标准窗口并跑 event loop
...
`0x81c004bc(index, table)` 从这个 table 取 resource pointer。return value 指向完整 `VX` 资源块，
...
`0x81c0240c` 加载 `.hx` 的文本 offset table：

1. 清零 `0x81c044d4` 开始的 0x74 byte。
2. 打开 `元素周期表.hx`。
3. seek 到固定位置读取 3 byte offset。
4. 循环 114 次读取 3 byte relative offset。
...
所以 custom `.hx` 的关键不是纯文本，而是要生成 3 byte offset table 和文本段。

## 4. 关键全局变量

```text
0x81c04370  主界面帮助文本 pointer -> 0x81c03850
0x81c04374  浏览界面帮助文本 pointer -> 0x81c03a94
...
重点：`GUI+0x030` 第二个参数在 Element 原厂代码中是 `0`，不是 frame handle。
这说明原机默认 frame/event slot 可以驱动这个应用，但不能推广为 SDK 通用规则。
新 no-template BDA 应优先使用 `bda_gui_event_poll_like(&msg, frame)`；只有在复刻
原机默认 slot 行为时才使用 `msg,0`。

## 7. 主 window proc `0x81c006d0`
...
`message == 1` 时，window proc 从 `lparam` 拆出坐标：
...
详情 window proc 里明确看到这些 command/键值：
...
## 8. 详情/浏览 window proc `0x81c014dc`
...
详情窗口创建同样使用 `GUI+0x2fc(15)` + `GUI+0x084(&desc)` + Element 原机 event loop。
...
这里的 return value 是 `VX` 开始地址。custom 程序如果使用 `GUI+0x540`，应保留完整资源块。
...
2. Element 原机 event loop 使用 GUI+0x030(msg, 0)；这不是 no-template SDK 的通用规则。
3. GUI+0x540 可以显示完整 VX 资源。
4. VX 参数是完整资源块，不是 pixels pointer。
5. DLX 可以由应用自己解析，不依赖系统 loader。
6. .hx 风格数据文件使用 3 byte offset table。
```
...
frame = create_frame(proc, surface=GUI+0x2fc(15));  // Element 原机样本，不是 no-template 默认
...
注意：上面的 `surface=GUI+0x2fc(15)` 和 `GUI+0x030(msg, 0)` 是 Element
原机应用在已有系统上下文中的模式。SDK 新程序先按 `window_notes.md` 使用
`bda_frame_desc_init_like(..., surface=0)`、`bda_gui_register_frame_desc_like()`
和 `bda_gui_event_poll_like(&msg, frame)` 收窄变量；需要复刻 Element 时再显式使用
原机 surface/default-slot 组合。
