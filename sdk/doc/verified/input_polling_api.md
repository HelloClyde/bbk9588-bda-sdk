...
bda_gui_key_pressed_like(packet, keycode);
...
int bda_gui_input_packet_key_pressed_like(
    const bda_gui_input_packet_like_t *packet,
    u32 keycode
);
int bda_gui_key_pressed_like(u32 keycode);
```
...
if (bda_gui_input_packet_key_pressed_like(&packet, BDA_KEY_LEFT)) {
    move_cursor_left();
}
if (bda_gui_input_packet_key_pressed_like(&packet, BDA_KEY_ENTER)) {
    open_current_item();
}
```
...
```c
if (bda_gui_key_pressed_like(BDA_KEY_ESCAPE)) {
    return 0;
}
```

若同一帧需要检查多个键，不要连续调用多次 `bda_gui_key_pressed_like()`；每次调用都会
重新采样。应先调用一次 `bda_gui_input_packet_like()`，再查询同一个 packet。
