...
| GUI | +0x1ac | `BDA_GUI_OBJECT_UPDATE3_LIKE` | `0x8028100c` | `0x800de150` | 是 | `addiu $sp, $sp, -0x28` | object update/layout；C200 构造 stack message packet 并同步发送内部 0x162。 |
| GUI | +0x1b0 | `BDA_GUI_OBJECT_UPDATE2_LIKE` | `0x80281010` | `0x800de190` | 是 | `addiu $sp, $sp, -0x28` | object update/layout；C200 构造 stack message packet 并同步发送内部 0x163。 |
| GUI | +0x1b4 | `BDA_GUI_OBJECT_PAIR_EXISTS_LIKE` | `0x80281014` | `0x800de0a8` | 是 | `move $t0, $zero` | object pair exists；C200 扫描 0x804a6b40 GUI 记录表，比较 record+0/record+4 后返回 0/1。 |
...
| GUI | +0x304 | `BDA_GUI_CURRENT_DRAW_LIKE` | `0x80281164` | `0x800bceec` | 是 | `addiu $sp, $sp, -0x18` | current draw context；C200 读取 handle，从 6 个 slot 取/初始化 context，并以 mode=0 调内部 helper。 |
| GUI | +0x308 | `BDA_GUI_BEGIN_DRAW_LIKE` | `0x80281168` | `0x800bce50` | 是 | `addiu $sp, $sp, -0x18` | begin draw context；C200 读取 handle，从 6 个 slot 取/初始化 context，并以 mode=1 调内部 helper。 |
| GUI | +0x30c | `BDA_GUI_END_DRAW_LIKE` | `0x8028116c` | `0x800bd4b0` | 是 | `addiu $sp, $sp, -0x20` | 结束 draw；C200 清理 draw context 状态，无稳定 return value。 |
| GUI | +0x310 | `BDA_GUI_COMPAT_CONTEXT_CREATE_LIKE` | `0x80281170` | `0x800bd100` | 是 | `addiu $sp, $sp, -0x30` | compatible draw context create；C200 分配 0xd4 byte context 并复制 source context 的 drawable bounds/backend。 |
| GUI | +0x314 | `BDA_GUI_SURFACE_FLUSH_LIKE` | `0x80281174` | `0x800bd584` | 是 | `addiu $sp, $sp, -0x18` | surface/canvas flush 并释放 context；C200 调 backend +0x34 后释放 object。 |
...
| GUI | +0x418 | `BDA_GUI_RENDER_FINISH_LIKE` | `0x80281278` | `0x800b3d90` | 是 | `addiu $sp, $sp, -0x60` | 双 context/双矩形 render helper；C200 使用 stack+0x14 的第二 context 并调用 backend +0x94。 |
...
| GUI | +0x6c8 | `BDA_GUI_FILE_SELECTOR_UPDATE_LIKE` | `0x80281528` | `0x80042fec` | 是 | `addiu $sp, $sp, -0x18` | file selector 更新/pump；C200 table entry 无参数。 |
...
| SYS | +0x040 | `BDA_SYS_PACKAGE_SOUND_OP40_LIKE` | `0x80280ca0` | `0x8018921c` | 是 | `slti $v0, $a0, 0` | 打包音效 low-level op40；C200 clamp a0 到 0..0x62，写 sound id 全局状态并置 pending flag。 |
| SYS | +0x044 | `BDA_SYS_PACKAGE_SOUND_OP44_LIKE` | `0x80280ca4` | `0x80189248` | 是 | `addiu $sp, $sp, -0x18` | 打包音效 low-level op44；C200 不读取参数，只调用内部 helper，无稳定 return value。 |
...
| SYS | +0x0a0 | `BDA_SYS_AUDIO_FLUSH_LIKE` | `0x80280d00` | `0x801891e8` | 是 | `addiu $sp, $sp, -0x18` | raw audio flush/drain；C200 无参数，连续调用 0x80195db0/0x80195db8/0x80195170，无稳定 return value。 |
...
| +0x00c | 2410 | 54 | GUI | `0x80280e6c` | `0x800dbe90` | `addiu $sp, $sp, -0x18` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x00c | 2410 | 54 | SYS | `0x80280c6c` | `0x80185814` | `addiu $sp, $sp, -0x70` | 已分析为 system resource scheduler helper，含 busy-wait，不公开 wrapper。 |
| +0x00c | 2410 | 54 | RES | `0x80280d3c` | `0x8013bc10` | `addiu $sp, $sp, -0x18` | 已分析为 resource descriptor/global state 写入 helper，不公开 wrapper。 |
...
| +0x004 | 1171 | 54 | GUI | `0x80280e64` | `0x800d4950` | `lui $t0, 0x8082` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x004 | 1171 | 54 | RES | `0x80280d34` | `0x8013aaf0` | `addiu $sp, $sp, -0x30` | 已分析为 resource manager 文件/cache 路径，不是 DLX loader，不公开 wrapper。 |
| +0x4f0 | 1086 | 46 | FS | `0x802812c0` | `0x800c0758` | `lw $v1, ($a0)` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x4f0 | 1086 | 46 | SYS | `0x80281150` | `0x800d3530` | `addiu $sp, $sp, -0x28` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x4f0 | 1086 | 46 | RES | `0x80281220` | `0x800bfb28` | `beqz $a0, 0x800bfb40` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
...
| +0x33c | 579 | 44 | FS | `0x8028110c` | `0x800c643c` | `addiu $sp, $sp, -0x20` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x33c | 579 | 44 | SYS | `0x80280f9c` | `0x800cae04` | `addiu $sp, $sp, -0x18` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x33c | 579 | 44 | RES | `0x8028106c` | `0x800c681c` | `addiu $sp, $sp, -0x30` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
...
| +0x338 | 485 | 45 | FS | `0x80281108` | `0x800c63ec` | `addiu $sp, $sp, -0x18` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x338 | 485 | 45 | SYS | `0x80280f98` | `0x800cd8d4` | `addiu $sp, $sp, -0x18` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x338 | 485 | 45 | RES | `0x80281068` | `0x800c65c0` | `addiu $sp, $sp, -0x48` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
...
| +0x08c | 398 | 54 | RES | `0x80280dbc` | `0x8001680c` | `addiu $sp, $sp, -0x2e8` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x2b8 | 381 | 44 | FS | `0x80281088` | `0x800c7df0` | `addiu $sp, $sp, -0x20` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x2b8 | 381 | 44 | SYS | `0x80280f18` | `0x800ce558` | `beqz $a0, 0x800ce570` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x2b8 | 381 | 44 | RES | `0x80280fe8` | `0x800d49d0` | `sll $v1, $a0, 2` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
...
| +0x0f4 | 244 | 17 | FS | `0x80280ec4` | `0x800cebb0` | `addiu $sp, $sp, -0x18` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x0f4 | 244 | 17 | SYS | `0x80280d54` | `0x80148a74` | `andi $a0, $a0, 0xff` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
| +0x0f4 | 244 | 17 | RES | `0x80280e24` | `0x8017a0d8` | `addiu $sp, $sp, -0x20` | 未分类 candidate：只能作为 disasm 导航，不能当作 SDK API。 |
