...
直接暴露成可玩游戏绘图框架。
`sdk/api/examples/tile_blit_probe.c` 现在只保留为 ABI/build probe：它用一次 draw guard
...
这说明当前缺失的是 shell 状态机，而不是单个 blit wrapper。下一步逆向重点应放在
`GUI+0x414/+0x418` render helper、`GUI+0x0e8` object draw end、`GUI+0x074`
present guard 和 `GUI+0x6e0` game/display state pump 之间的状态关系。
