...
是/全部/否确认框用于批量操作。三个按钮从左到右固定返回 `YES`、`ALL` 和 `NO`：

```c
int result = bda_confirm_yes_all_no("Save", "Save this item?");
if (result == BDA_DIALOG_RESULT_YES) {
    /* 只处理当前项。 */
} else if (result == BDA_DIALOG_RESULT_ALL) {
    /* 处理当前项和剩余项。 */
} else if (result == BDA_DIALOG_RESULT_NO) {
    /* 跳过当前项。 */
}
...
![系统是/全部/否确认框](assets/msgbox_yes_all_no_verified.png)
...
- 是/全部/否确认使用 `parent=0, flags=BDA_MSGBOX_TYPE_YES_ALL_NO`，即
  `bda_confirm_yes_all_no()`。
- 8013 已动态确认“是”返回 `6`、“否”返回 `7`；不能把返回值直接当作布尔值，
  因为两个结果都非零。
- 三按钮布局的“全部”返回 `10`，三个结果都不能直接当作布尔值。
- 非空 `parent`、按实体退出键关闭确认框时的返回值，以及除 `0/2/6` 外的 `flags`
  仍未验证。
...
example/system/three_button_dialog/ThreeButtonDialog.bda
```

探针连续调用三次 `bda_confirm_yes_all_no()`，依次点击左、中、右按钮。8013 显示的
固件按钮文字为“是 / 全部 / 否”，不是“是 / 否 / 取消”：

![三按钮布局](assets/msgbox_yes_all_no_verified.png)

每次调用返回后，探针立即关闭并重开 `A:\DIALOG6.LOG` 追加结果。导出的日志为：
...
![三按钮返回值闭环](assets/msgbox_yes_all_no_result_verified.png)

测试时 QEMU 在三次模态调用和最终结果页期间保持 `running=true`、
`stop_reason=null`、`invalid=[]`，关闭结果页后正常返回菜单。当前预编译 BDA 大小为
`0x9aa0`，SHA-256 为
`5420737A27261C465A80696B7CFEC7514561065336B5162A3351E5CAF76F8CCE`。原始日志保存在
[`assets/msgbox_yes_all_no_probe_log.txt`](assets/msgbox_yes_all_no_probe_log.txt)。

## 验证边界

本页证明 standalone header、VX icon、flat code loader 和 `GUI+0x2b8` 的单按钮、
是/否及是/全部/否模态路径可以共同工作。它不证明其他 GUI wrapper、window
lifecycle、确认框的退出键返回值或尚未映射的其他布局已经可用。
