...
`sdk/include/bda_sdk.h` 是基础公开头，`sdk/include/bda_controls.h` 和
`sdk/include/bda_audio.h` 分别是控件与 raw PCM 公开头。这里不是逆向 API 候选目录，
只能放入已经动态验证、ABI 稳定且结论确定的系统或固件绑定 API。
...
- Message Box：`docs/verified/msgbox_api.md`
